...
def get_split(split_name, dataset_dir, file_pattern, reader,
    ...
    :param file_pattern: The file pattern to use when matching the dataset sources.
        It is assumed that the pattern contains a `%s` string so that the split name
        can be inserted.
    :param reader: The TensorFlow reader type.
    :return:
        A `Dataset` namedtupe.
    :raise:
        ValueError: if `split_name` is not valid train/test split.
    """
    ...
    items_to_handlers = {
        ...
        'object/bbox': slim.tfexample_decoder.BoundingBox(
            ['ymin', 'xmin', 'ymax', 'xmax'], 'image/object/bbox/label'),
